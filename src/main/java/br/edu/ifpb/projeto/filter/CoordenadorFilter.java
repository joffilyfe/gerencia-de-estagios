...
@WebFilter(filterName = "CoordenadorFilter", urlPatterns = {"/view/coordenador/*"})
	...
	public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain) throws IOException, ServletException {
		...
		if (httpSession != null) {
			...
			if (loginUser == null) {
				...
				return;
			}else if (!loginUser.isCoordenador()){
				// Se usuario logado nao for coordenador
				httpSession.setAttribute("message", "Usuario deve ser coordenador para acessar pagina!");
				redirectIndex(httpRequest, httpResponse);
				return;
			}
		} else {
